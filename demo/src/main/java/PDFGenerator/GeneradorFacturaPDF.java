...
public class GeneradorFacturaPDF extends AbstractGeneradorPDF {
    ...
    izquierda.addElement(new Paragraph(empresa.getNombre(), new Font(Font.FontFamily.HELVETICA, 24, Font.BOLD, new BaseColor(180, 0, 0))));
    ...
    Paragraph titulo = new Paragraph("FACTURA", new Font(Font.FontFamily.HELVETICA, 20, Font.BOLD, BaseColor.WHITE));
...
public void agregarDatosCliente(Cliente cliente, Comprobante comprobante) throws Exception {
    ...
    left.setPadding(8);
    left.addElement(new Paragraph("Facturar a:", new Font(Font.FontFamily.HELVETICA, 11, Font.BOLD, new BaseColor(0, 0, 0))));
    ...
    right.setPadding(8);
    right.addElement(new Paragraph("N° de factura: " + comprobante.getSerie(), new Font(Font.FontFamily.HELVETICA, 11, Font.BOLD, new BaseColor(180, 0, 0))));
...
public void agregarCuerpo(Comprobante comprobante, List<DetalleComprobante> detalles) throws Exception {
    ...
    for (String enc : encabezados) {
        PdfPCell c = new PdfPCell(new Paragraph(enc, new Font(Font.FontFamily.HELVETICA, 10, Font.BOLD, BaseColor.WHITE)));
    ...
    for (DetalleComprobante d : detalles) {
        ...
        table.addCell(celdaConColor(d.getNombreProducto() != null ? d.getNombreProducto() : "-", fontNormal, fondo));
    ...
    resumen.addCell(celda("", fontNormal));
    PdfPCell totalCell = celdaConBorde("TOTAL: " + currency.format(total), new Font(Font.FontFamily.HELVETICA, 11, Font.BOLD, new BaseColor(180, 0, 0)));
